...
public class GlavniEkran extends JFrame
{
    ...
    private List<Racun> _listaRacuna;
    ...
    public GlavniEkran()
    {
        ...
        _listaOsoba = new ArrayList<Osoba>();
        _listaRacuna = new ArrayList<Racun>();
    ...
    private void createMenuBar()
    {
        ...
        itemDodajOsobu.addActionListener(new ActionListener()
        {
            ...
            {
                UnosenjeNoveOsobeFrame frame = new UnosenjeNoveOsobeFrame(_listaOsoba);
                frame.prikaziEkran();
            }
        });
        ...
        itemDodajRacun.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent event)
            {
                UnosenjeNovogRacunaFrame frame = new UnosenjeNovogRacunaFrame(_listaOsoba, _listaRacuna);
    ...
    private void createMainWindow()
    {
        ...
        _comboBoxPrviRacun.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent event)
            {
                ...
                if(prviRacun == null) prviRacun = _listaRacuna.get(0);
        ...
        _comboBoxDrugiRacun.addActionListener(new ActionListener()
        {
            public void actionPerformed(ActionEvent event)
            {
                ...
                if(drugiRacun == null) drugiRacun = _listaRacuna.get(0);
    ...
    {
        int rowCount = ((DefaultTableModel)_tablePopisTransakcija.getModel()).getRowCount();
        if (rowCount > 0)
        {
            String iznosZadnjeTransakcije =
                (((DefaultTableModel)_tablePopisTransakcija.getModel()).getValueAt(rowCount - 1, 3)).toString();
            BigDecimal pravaVrijednost = new BigDecimal(iznosZadnjeTransakcije);

            String obavijest;
            if (pravaVrijednost.compareTo(iznos) > 0)
            {
                obavijest = "Iznos transakcije je manji od zadnje transakcije.";
            }
            else if (pravaVrijednost.compareTo(iznos) < 0)
            {
                obavijest = "Iznos transakcije je veći od zadnje transakcije.";
            }
            else
            {
                obavijest = "Iznos transakcije je jednak zadnjoj transakciji.";
            }

            JOptionPane.showMessageDialog(null, obavijest);
        }
